...
@Component({
  selector: 'app-new-acceptance-detailed-report',
  templateUrl: './new-acceptance-detailed-report.component.html',
  styleUrls: ['./new-acceptance-detailed-report.component.css'],
  providers: [DatePipe],
})
export class NewAcceptanceDetailedReportComponent {
  ...
  convertInExcel() {
    ...
    if (this.dataList11.length > 0) {
      for (var i = 0; i < this.dataList11.length; i++) {
        ...
        obj1['Filled Datetime'] = this.dataList11[i]['ALLOTMENT_DATE_TIME'];
        obj1['Non-Acceptance Remark'] =
          this.dataList11[i]['NON_ACCEPTANCE_REMARK'];
        obj1['Non-Acceptance Datetime'] =
          this.dataList11[i]['NON_ACCEPTANCE_END_DATE_TIME'];
        if (this.dataList11[i]['PHYSICAL_POSSESSION_DATE']) {
          obj1['Bard Due to No Action'] = 'Yes';
        } else {
          obj1['Bard Due to No Action'] = 'No';
        }
