...
@Component({
  selector: 'app-nonacceptanceamp',
  templateUrl: './nonacceptanceamp.component.html',
  styleUrls: ['./nonacceptanceamp.component.css'],
  providers: [DatePipe]
})
export class NonacceptanceampComponent implements OnInit {
  @Input() empid: any;
  @Input() employerecordfilled!: boolean;
  @Input() isdraweropendedggg: boolean = false;
  @Input() drawerClose;
  @Input() stage!: number;
  @Input() data: any = new DataNonAcceptanceMaster();
  @Input() dataass: any = new DataNonAcceptanceMaster();
    ...
    if (this.isdraweropendedggg == true) {
      this.api
        .getnonaccetances(
          ...
          'ID',
          'desc',
          ' AND FINAL_FLAT_ALLOTMENT_DETAILS_ID = ' + this.data.ID + ' AND EMP_ID = ' + this.empid
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
              ...
              if (data['data'].length > 0) {
                this.sampletext = data['data'][0]['NON_ACCEPTANCE_TEXT'];
              }
            }
          },
          (err) => {
  ...
  downloadPdfdraft() {
    ...
    const opt = {
      margin: 0.2,
      filename: 'Non Acceptance Form.pdf',
  ...
  previous2() {
    ...
    this.api
      .getnonaccetances(
        ...
        'ID',
        'desc',
        ' AND FINAL_FLAT_ALLOTMENT_DETAILS_ID = ' + this.dataass.ID + ' AND EMP_ID = ' + this.empid
      )
      .subscribe(
        (data) => {
          this.data = data['data'][0];
  ...
  nextUpload() {
    if(this.sampletext.length<=0){
      ...
      if (this.datacount > 0) {
        this.data.NON_ACCEPTANCE_STEP_NO = 0;
        this.data.NON_ACCEPTANCE_PRINT_DATE_TIME = this.datepipe.transform(new Date(), 'yyyy-MM-dd HH:mm:ss');
        this.data.EMP_ID = this.data.EMP_ID;
        this.data.FLAT_ID = this.data.FLAT_ID;
        // this.data.FINAL_FLAT_ALLOTMENT_DETAILS_ID = this.data.ID;
        this.data.NON_ACCEPTANCE_TEXT = this.sampletext;
  this.savenextapp=true;
        this.api.updatenonacceptances(this.data).subscribe((successCode) => {
        ...
        this.dataass.NON_ACCEPTANCE_STEP_NO = 0;
        this.dataass.NON_ACCEPTANCE_PRINT_DATE_TIME = this.datepipe.transform(new Date(), 'yyyy-MM-dd HH:mm:ss');
        this.dataass.EMP_ID = this.dataass.EMP_ID;
        this.dataass.FLAT_ID = this.dataass.FLAT_ID;
        this.dataass.FINAL_FLAT_ALLOTMENT_DETAILS_ID = this.dataass.ID;
        this.dataass.CLIENT_ID = 1;
        this.dataass.NON_ACCEPTANCE_TEXT = this.sampletext;
        this.savenextapp=true;
  
        this.api.Createnonacceptances(this.dataass).subscribe((successCode) => {
          if (successCode['code'] == '200') {
            ...
            this.api
              ...
              .subscribe(
                (data) => {
                  ...
                  if (this.data.NON_ACCEPTANCE_FORM != null) {
  ...
  onFileApplicationform(event: any) {
    if (event.target.files[0].type == 'application/pdf') {
      ...
      if (this.appnPDF != null) {
        ...
        this.urlappnPdf = url;
        if (
          this.data.NON_ACCEPTANCE_FORM != undefined &&
          this.data.NON_ACCEPTANCE_FORM.trim() != ''
        ) {
          var arr = this.data.NON_ACCEPTANCE_FORM.split('/');
          if (arr.length > 1) {
            url = arr[5];
          }
        }
      ...
      this.timer = this.api
        .onUpload2('nonAcceptanceLetter', this.appnPDF, this.urlappnPdf)
        .subscribe((res) => {
          ...
          if (res.type === HttpEventType.UploadProgress) {
            ...
            if (this.percent == 100) {
              this.isSpinning = false;
      this.saveformnon=false;

              // this.message.success('File Uploaded Successfully', '')
            }
          } else if (res.type == 2 && (res.status != 200)) {
            this.message.error('Failed To Upload Non-Acceptance Form ', '');
            this.progressBar = false;
            this.percent = 0;
            this.data.NON_ACCEPTANCE_FORM = null;
      this.saveformnon=false;

          } else if (res.type == 4 && res.status == 200) {
            if (res.body['code'] == 200) {
              this.data.NON_ACCEPTANCE_FORM = this.urlappnPdf;
              this.message.success('Non-Acceptance Form Uploaded Successfully', '');
      this.saveformnon=false;
  ...
  Submitapplication() {
    if (
      this.data.NON_ACCEPTANCE_FORM == null ||
      this.data.NON_ACCEPTANCE_FORM == undefined ||
      this.data.NON_ACCEPTANCE_FORM == ''
    ) {
      this.message.error('Please Upload Application Form', '');
    } else {
      // this.data.STATUS = 'P';
      // this.data.NON_ACCEPTANCE_STEP_NO = 2;
      // this.data.NON_ACCEPTANCE_FORM_SUBMITTED_DATE_TIME = this.datepipe.transform(
      //   new Date(),
      //   'yyyy-MM-dd HH:mm:ss'
      // );
      this.data.IS_NON_ACCEPTANCE_FORM_SUBMITTED = 1;
      this.data.NON_ACCEPTANCE_STEP_NO = 1;
      this.data.NON_ACCEPTANCE_FORM_SUBMITTED_DATE_TIME = this.datepipe.transform(new Date(), 'yyyy-MM-dd HH:mm:ss');
      this.data.EMP_ID = this.data.EMP_ID;
      this.data.FLAT_ID = this.data.FLAT_ID;
      this.data.FINAL_FLAT_ALLOTMENT_DETAILS_ID = this.dataass.ID;
      this.submitapplnon=true;
      if(this.dataass.NON_ACCEPTANCE_INSPECTOR_STATUS=='R'){
        this.data.NON_ACCEPTANCE_INSPECTOR_STATUS='P';
        this.data.NON_ACCEPTANCE_INSPECTOR_REMARK=null;
        this.data.NON_ACCEPTANCE_INSPECTOR_APPROVED_DATE_TIME=null;
      }

      this.api.updatenonacceptances(this.data).subscribe((successCode) => {
        if (successCode['code'] == '200') {
          this.message.success('Non-Acceptance Form Submitted Successfully...', '');
      ...
      this.submitapplnon=false;
      this.message.error('Failed to submit Non-Acceptance form', '');
  ...
  handleOkform(): void {
    if (
      ...
      this.message.error('Please Upload Non-Acceptance Form', '');
