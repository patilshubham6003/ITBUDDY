...
@Component({
  selector: 'app-senioritysummaryreport',
  templateUrl: './senioritysummaryreport.component.html',
  styleUrls: ['./senioritysummaryreport.component.css'],
  providers: [DatePipe],
})
export class SenioritysummaryreportComponent {
  formTitle = 'Seniority Summary';
  ...
  ExcelList: any = [];
  query: any;
  columns: string[][] = [['RESIDENCE_TYPE', 'Residence Type']];
  constructor(
  ...
  // fileName = 'Seniority Summary Report';
  ...
  convertInExcel() {
    ...
    if (this.ExcelList.length > 0) {
      for (var i = 0; i < this.ExcelList.length; i++) {
        obj1['Residence Type'] = this.ExcelList[i]['RESIDENCE_TYPE'];
        ...
        obj1['Count'] = this.ExcelList[i]['TOTAL_COUNT'];

        arry1.push(Object.assign({}, obj1));
        if (i == this.ExcelList.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Seniority Summary Report' +
