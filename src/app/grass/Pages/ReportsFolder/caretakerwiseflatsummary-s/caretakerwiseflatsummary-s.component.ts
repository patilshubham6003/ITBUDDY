...
@Component({
  selector: 'app-caretakerwiseflatsummary-s',
  templateUrl: './caretakerwiseflatsummary-s.component.html',
  styleUrls: ['./caretakerwiseflatsummary-s.component.css'],
  providers: [DatePipe],
})
export class CaretakerwiseflatsummarySComponent implements OnInit {
  formTitle = 'Caretaker Wise Quarter Summary';
  dataList: any[] = [];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    var caretackername = '';
    if (
      this.CARETAKER_ID != null &&
      this.CARETAKER_ID != undefined &&
      this.CARETAKER_ID != ''
    ) {
      var stringcaretaker = this.CARETAKER_ID.join(',');
      caretackername = ' AND CARETAKER_ID IN ( ' + stringcaretaker + ' )';
    }
    if (exportInExcel == false) {
      ...
      this.api
        .caretakerWiseFlatSummarySReport(
          ...
          this.sortKey,
          sort,
          likeQuery + filterQuery1 + Areaname + caretackername
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  convertInExcel() {
    ...
    if (this.dataList11.length > 0) {
      for (var i = 0; i < this.dataList11.length; i++) {
        ...
        obj1['Caretaker Name'] = this.dataList11[i]['CARETAKER_NAME'];
        ...
        if (i == this.dataList11.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Caretaker Wise Quarter Summary ' +
