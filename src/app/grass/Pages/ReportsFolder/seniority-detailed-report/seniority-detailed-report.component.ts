...
@Component({
  selector: 'app-seniority-detailed-report',
  templateUrl: './seniority-detailed-report.component.html',
  styleUrls: ['./seniority-detailed-report.component.css'],
  providers: [DatePipe],
})
export class SeniorityDetailedReportComponent implements OnInit {
  formTitle = 'Seniority Detailed Report';
  ...
  columns: string[][] = [
    ...
    ['DOB', 'DOB'],
    ['CAST', 'CAST'],
    ['DATE_OF_PRIORITY', 'DATE_OF_PRIORITY'],
  ...
  applyFilter() {
    ...
    if (
      ...
      this.DateOfPriority != undefined &&
      this.DateOfPriority != null
    ) {
      this.filterQuery +=
        " AND ( DATE_OF_PRIORITY BETWEEN '" +
        this.DateOfPriority1 +
    ...
    //   this.DateOfjoining != undefined &&this.DateOfjoining !=null
    // ) {
    //   this.filterQuery +=
    ...
    if (this.EMPLOYEE != null && this.EMPLOYEE != '') {
      ...
      this.filterQuery += " AND EMPLOYEE_NAME='" + this.EMPLOYEE + "'";
    } else {
      this.filterQuery = this.filterQuery;
    }
    ...
    if (
      ...
      // this.filterQuery += " AND RESIDENCE_TYPE_ID IN (" + this.ResidenceType + ")";
      this.filterQuery += " AND RESIDENCE_TYPE='" + this.ResidenceType + "'";
    } else {
      this.filterQuery = this.filterQuery;
    }
    ...
    if (
      ...
      this.filterQuery += " AND GRADE_PAY_LEVEL='" + this.GradePayLevel + "'";
    } else {
      this.filterQuery = this.filterQuery;
    }
  ...
  convertInExcel() {
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        ...
        obj1['Residence Type'] = this.excelData[i]['RESIDENCE_TYPE'];
