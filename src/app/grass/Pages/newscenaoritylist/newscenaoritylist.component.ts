...
@Component({
  selector: 'app-newscenaoritylist',
  templateUrl: './newscenaoritylist.component.html',
  styleUrls: ['./newscenaoritylist.component.css'],
  providers: [DatePipe],
})
export class NewscenaoritylistComponent implements OnInit {
  isVisible = false;
  sanitizedLink: any = '';
  sanitizedLink1: any = '';
  ...
  url: any;
  folderName = 'seniorityObjections';
  ...
  raiseObjection(data) {
    ...
    this.objectiondata.DRAFT_WAITING_MASTER_ID = data.ID;
  ...
  // ngOnInit() {
  //   this.month1 = this.nowdate.getMonth() + 1;
  ...
  //           this.allotmentdata = data['data'][0];

  //           if (this.allotmentdata.STATUS == 'A') {
  //             this.api
  ...
  //                 'RESIDENCE_TYPE',
  //                 'asc',
  //                 ' AND FLAT_REQUEST_ID = ' + this.allotmentdata.ID
  //               )
  //               .subscribe((data) => {
  //                 if (data['code'] == 200) {
  ...
  //                   let aar: any = [];
  //                   let stringvalue = '';
  //                   if (this.flatarrlist.length > 0) {
  //                     for (let a = 0; a < this.flatarrlist.length; a++) {
  //                       aar.push(this.flatarrlist[a]['RESIDENCE_TYPE']);
  //                     }
  //                     if (aar.length <= 1) {
  //                       stringvalue = aar[0];
  //                     } else {
  //                       stringvalue = aar.join(',');
  //                     }
  //                   }
  ...
  //                       (data) => {
  //                         this.listview = data['data'];
  //                         this.showHideReasonArray = new Array(
  //                           this.listview.length
  //                         ).fill(false);
  //                         this.isRecordLoading = false;
  //                         if (this.listview.length <= 0) {
  //                           this.message.info('No Order Generated Yet.', '');
  ...
  //                             // let sapandate = this.listview[ab]['OBJ_END_TIME_PERIOD'];
  //                             let startdate =
  //                               this.listview[ab]['OBJ_START_TIME_PERIOD'];
  //                             let targetDate = new Date(startdate); // Your datetime value should be assigned here
  ...
  //                           for (let cc = 0; cc < this.listview.length; cc++) {
  //                             let sapandate =
  //                               this.listview[cc]['OBJ_END_TIME_PERIOD'];
  //                             let targetDate = new Date(sapandate); // Your datetime value should be assigned here
  //                             let startdate =
  //                               this.listview[cc]['OBJ_START_TIME_PERIOD'];
  //                             const startdatet = new Date(startdate);
  ...
  // getobjectionmastertt(): any {
  //   this.api.getObjectionMaster(0, 0, 'ID', 'desc', ' AND EMPLOYEE_ID=' + this.employeeID + " AND FLAT_REQUEST_ID=" + this.allotmentdata.ID).subscribe((data) => {
  //     if (data['code'] == 200) {
  ...
  drawerData;
  final;
  // showfinal(data:ObjectionMasterNew,pdfURL: string,event: any)
  // {
  //   this.drawerData = Object.assign({}, data);
  //     //
  //     this.api.getfinalSenioritylist(
  //       0,
  //       0,
  //       '',
  //       'asc',
  //       ' AND DRAFT_WAITING_MASTER_ID = ' +event.ID
  ...
  //     a = `${this.api.retriveimgUrl}finalList/${this.final}`
  ...
  getS(link: string) {
    var a = '';
    // if (this.view == 1) {
    a = this.api.retriveimgUrl + 'draftWaitingList/' + link;
    // }
  ...
  showfinal(data: ObjectionMasterNew, event: any) {
    this.drawerData = Object.assign({}, data);
    //
    this.api
  ...
  showmobfile() {
    const fileUrl = this.api.retriveimgUrl + 'finalList/' + this.final;
    window.open(fileUrl);
  }
  ...
  search() {
    ...
    if (
      ...
      this.api
        .getSenioritylistdraftchange(0, 0, '', 'asc', this.Filterquery)
        .subscribe(
          (data) => {
            this.listview = data['data'];
            this.filterClass = 'filter-invisible';
