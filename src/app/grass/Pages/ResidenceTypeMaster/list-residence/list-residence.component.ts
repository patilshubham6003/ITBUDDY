...
export class ListResidenceComponent implements OnInit {
  ...
  drawerTitle!: string;
  drawerData: ResidenceMaster = new ResidenceMaster();
  formTitle = 'Residence Master';
  dataList = [];
  ...
  add(): void {
    this.drawerTitle = 'Add Residence Information';
    this.drawerData = new ResidenceMaster();

    this.api.getResidence(1, 1, 'SEQUENCE_NO', 'desc', '').subscribe(
      (data) => {
        if (data['count'] == 0) {
  ...
  edit(data: ResidenceMaster): void {
    this.drawerTitle = 'Update Residence Information';
