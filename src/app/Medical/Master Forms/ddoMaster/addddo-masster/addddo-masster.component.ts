...
@Component({
  selector: 'app-addddo-masster',
  templateUrl: './addddo-masster.component.html',
  styleUrls: ['./addddo-masster.component.css'],
})
export class AddddoMassterComponent {
  isSpinning = false;
  ...
  loadSignatures() {
    ...
    // change get
    this.api.getSignature(0, 0, '', '', ' AND STATUS!=false').subscribe(
      (data) => {
        if (data['code'] == 200) {
  ...
  selectchange2(key: any) {
    ...
    if (key != null || key != undefined) {
      this.api
        .getSignature(0, 0, '', '', '' + 'AND STATUS = 1 AND ID=' + key)
        .subscribe((data) => {
          if (data['code'] == 200) {
            this.data2.SIGNATURE_NAME = data['data'][0]['NAME'];
          } else {
          }
        });
    }
    ...
    if (isOK) {
      if (this.SignatureData.length > 0) {
        for (let i = 0; i < this.SignatureData.length; i++) {
          if (this.data2.SIGNATURE_ID == this.SignatureData[i].SIGNATURE_ID) {
  ...
  save(addNew: boolean, designation: NgForm): void {
    ...
    if (
      ...
      this.data.HEAD_OF_OFFICE_HN == '' ||
      this.data.HEAD_OF_OFFICE_HN == null ||
      this.data.HEAD_OF_OFFICE_HN == undefined
    ) {
      this.isOk = false;
      this.message.error(' Please Enter Head Of Office In Hindi ', '');
    } else if (
      this.data.LOCATION == '' ||
