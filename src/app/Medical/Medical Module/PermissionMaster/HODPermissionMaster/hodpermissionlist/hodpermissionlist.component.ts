...
@Component({
  selector: 'app-hodpermissionlist',
  templateUrl: './hodpermissionlist.component.html',
  styleUrls: ['./hodpermissionlist.component.css'],
})
export class HODPermissionlistComponent implements OnInit {
  drawerData2: any[] = [];
  drawerVisible: boolean = false;
  drawerTitle: string = '';
  filterClass: string = 'filter-invisible';

  drawerData: HODPermissionMaster = new HODPermissionMaster();
  ...
  add(): void {
    this.drawerTitle = 'Create New HOD Permission';
    this.drawerData = new HODPermissionMaster();
    this.currentStage = 0;
    this.isSpin = false;
  ...
  HODPermissionID;
  edit(data: any): void {
    this.drawerTitle = 'Edit HOD Permission Details';
    this.drawerData = Object.assign({}, data);
    this.HODPermissionID = '';
    this.HODPermissionID = this.drawerData.ID;
  ...
  deleteConfirm(data: any) {
    ...
    this.api.deleteHODPermissionMaster(data1).subscribe((successCode) => {
  ...
  empID1;
  claimID1;
  hodOrderSignatureData: any = [];
  orderDrawer(data) {
    this.loadingRecords = true;
    this.hodOrderTitle = 'HOD Permission Order';
    this.hodDrawerData = Object.assign({}, data);
    if (
      this.hodDrawerData.SIGNATURE_ID != undefined &&
      this.hodDrawerData.SIGNATURE_ID != null
    ) {
      this.api
        .getSignature(
          0,
          ...
          'ID',
          'desc',
          ' AND ID = ' + this.hodDrawerData.SIGNATURE_ID
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
    ...
    this.api
      .getHospitalMapping(
        ...
        '',
        '',
        ' AND HOD_ID = ' + this.hodDrawerData.ID
      )
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
                ...
                (data) => {
                  if (data['code'] == 200) {
                    if (data['data'].length > 0) {
                      ...
                      this.hodOrderSignatureData = new MedicalSignatureData();
                    }
                    this.loadingRecords = false;
                    this.hodOrderVisible = true;
                  } else {
  ...
  hodDrawerData: any = new HODPermissionMaster();
  hodLetterSignatureData: any = [];
  letterDrawer(data) {
    this.loadingRecords = true;
    this.hodLetterTitle = 'HOD Permission Letter ';
    this.hodDrawerData = Object.assign({}, data);
    if (this.hodDrawerData.LETTER_HEADER) {
      this.hodDrawerData.LETTER_HEADER = this.hodDrawerData.LETTER_HEADER;
    } else {
      this.hodDrawerData.LETTER_HEADER =
            ...
            this.api
              ...
              .subscribe(
                (data) => {
                  if (data['code'] == 200) {
                    if (data['data'].length > 0) {
                      this.hodLetterSignatureData = data['data'][0];
                    } else {
                      this.hodLetterSignatureData = new MedicalSignatureData();
                    }
                    this.loadingRecords = false;
                    this.hodLetterVisible = true;
                  } else {
