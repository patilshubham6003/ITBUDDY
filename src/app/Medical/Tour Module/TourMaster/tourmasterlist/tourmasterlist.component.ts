...
@Component({
  selector: 'app-tourmasterlist',
  templateUrl: './tourmasterlist.component.html',
  styleUrls: ['./tourmasterlist.component.css'],
})
export class TourmasterlistComponent implements OnInit {
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
    ...
    if (
      ...
      this.api
        .getSignature(
          ...
          '',
          '',
          ' AND STATUS = 1 AND ID = ' + this.checklist.SIGNATURE_ID
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  search(reset: boolean = false) {
    ...
    if (this.isAdmin) {
      this.extraFilterQuery = ' AND IS_APPLYING_FOR_ADVANCE =0';
    } else {
      ...
      this.extraFilterQuery =
        " AND TOUR_STATUS <>'P' AND IS_APPLYING_FOR_ADVANCE =0 " + tempFilter;
    }
    this.api
      .gettouralldata(
        this.pageIndex,
