...
@Component({
  selector: 'app-mode-wise-amount-stat',
  templateUrl: './mode-wise-amount-stat.component.html',
  styleUrls: ['./mode-wise-amount-stat.component.css'],
})
export class ModeWiseAmountStatComponent implements OnInit {
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  columns: string[][] = [['NAME', 'Travel Mode Name']];
  ...
  convertInExcel() {
    ...
    if (this.inspector.length > 0) {
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        obj1['TTravel Class Name'] = this.inspector[i]['TRAVEL_CLASS_NAME'];
