...
@Component({
  selector: 'app-advance-given-for-journeyltc',
  templateUrl: './advance-given-for-journeyltc.component.html',
  styleUrls: ['./advance-given-for-journeyltc.component.css'],
})
export class AdvanceGivenForJourneyltcComponent implements OnInit {
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.searchText != '') {
      likeQuery = " AND NAME like '%" + this.searchText + "%'";
    }
  ...
  convertInExcel() {
    ...
    if (this.inspector.length > 0) {
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        obj1['Is Advance Taken'] = this.inspector[i]['IS_ADVANCE_TAKEN'];
        obj1[' Amount of Advance(₹)'] = this.inspector[i]['AMOUNT_OF_ADVANCE'];
