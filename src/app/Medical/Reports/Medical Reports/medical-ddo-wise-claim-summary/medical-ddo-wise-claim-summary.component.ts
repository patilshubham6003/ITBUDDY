...
@Component({
  selector: 'app-medical-ddo-wise-claim-summary',
  templateUrl: './medical-ddo-wise-claim-summary.component.html',
  styleUrls: ['./medical-ddo-wise-claim-summary.component.css'],
})
export class MedicalDdoWiseClaimSummaryComponent {
  formTitle = 'Medical DDO Wise Claim Summary Report';
  userId: any;
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  applyFilter() {
    ...
    if (this.CLAIM_AT) {
      this.filterQuery += ' AND IS_ADMIN_CLAIM = ' + this.CLAIM_AT;
    }
      ...
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Medical DDO Wise Claim Summary Report' +
