...
@Component({
  selector: 'app-user-wise-files-summary-report',
  templateUrl: './user-wise-files-summary-report.component.html',
  styleUrls: ['./user-wise-files-summary-report.component.css'],
})
export class UserWiseFilesSummaryReportComponent implements OnInit {
  formTitle = 'User Wise Files Summary Report';
  ...
  columns: string[][] = [
    ...
    ['PENDING_AT_ME', 'Pending At Me'],
    ['PENDING_AT_ME', 'Pending At Others'],
    ['IN_ADVANCE', 'In Advanced'],
    ['IN_FINAL', 'In Final'],
    ['TRANSFERED', 'Closed'],
  ];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.searchText != '') {
      likeQuery = " AND CREATOR_NAME like '%" + this.searchText + "%'";
      ...
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'User Wise Files Summary Report' +
