...
@Component({
  selector: 'app-medical-ddo-wise-claim-detailed',
  templateUrl: './medical-ddo-wise-claim-detailed.component.html',
  styleUrls: ['./medical-ddo-wise-claim-detailed.component.css'],
})
export class MedicalDdoWiseClaimDetailedComponent {
  formTitle = 'Medical DDO Wise Claim Detailed Report';
  userId: any;
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  columns: string[][] = [
    ...
    ['RELATION_WITH_PATIENT', 'Relationship With Patient'],
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (exportInExcel == false) {
      ...
      this.api
        .getclaimMaster2(0, 0, this.sortKey, sort, this.filterQuery + likeQuery)
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
              this.exportLoading = false;
  ...
  convertInExcel() {
    ...
    if (this.inspector.length > 0) {
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        obj1['Email ID'] = this.inspector[i]['EMAIL_ID'];
        ...
        obj1['Patient Name'] = this.inspector[i]['PATIENT_NAME'];
        obj1['Relationship With Patient'] =
          this.inspector[i]['RELATION_WITH_PATIENT'];
        ...
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Medical DDO Wise Claim Detailed Report' +
