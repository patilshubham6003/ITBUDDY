...
@Component({
  selector: 'app-hierarchy-wise-files-summary-report',
  templateUrl: './hierarchy-wise-files-summary-report.component.html',
  styleUrls: ['./hierarchy-wise-files-summary-report.component.css'],
})
export class HierarchyWiseFilesSummaryReportComponent implements OnInit {
  formTitle = 'Hierarchy Wise Files Summary Report';
  ...
  columns: string[][] = [['HIRARCHY_NAME', 'Hirarchy Name']];
  ...
  convertInExcel() {
    ...
    if (this.inspector.length) {
      for (var i = 0; i < this.inspector.length; i++) {
        obj1['Hierarchy Name'] = this.inspector[i]['HIRARCHY_NAME'];

        obj1['Created'] = this.inspector[i]['CREATED'];
        obj1['Transferred To Other'] = this.inspector[i]['TRANSFERED'];
        obj1['Pending at my desk'] = this.inspector[i]['PENDING_AT_ME'];
        obj1['Pending at other desk'] = this.inspector[i]['PENDING_AT_OTHERS'];
        obj1['In Advanced'] = this.inspector[i]['IN_ADVANCE'];
        obj1['In Final'] = this.inspector[i]['IN_FINAL'];
        obj1['Closed'] = this.inspector[i]['CLOSED'];
        arry1.push(Object.assign({}, obj1));
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Hierarchy Wise File Summary Report' +
