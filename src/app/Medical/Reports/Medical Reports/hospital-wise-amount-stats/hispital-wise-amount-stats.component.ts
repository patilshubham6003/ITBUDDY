...
@Component({
  selector: 'app-hispital-wise-amount-stats',
  templateUrl: './hispital-wise-amount-stats.component.html',
  styleUrls: ['./hispital-wise-amount-stats.component.css'],
})
export class HispitalWiseAmountStatsComponent implements OnInit {
  formTitle = 'Hospital Wise Amount Stats Report';
  ...
  columns: string[][] = [
    ...
    ['CLAIMED_AMOUNT', 'Total Amount of reimbursement claimed'],
    ['ADMISSIBLE_AMOUNT', ' Total Amount of reimbursement Admissible'],
    ['ADVANCE_AMOUNT', 'Total Advance Given Amount'],
    ['NET_PAYABLE_AMOUNT', 'Total Net amount Payable/recoverable'],
  ];
  ...
  applyFilter() {
    ...
    if (this.HOSPITAL_TYPE != undefined) {
      this.HOSPITAL_TYPE = this.HOSPITAL_TYPE;
    }
      ...
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Hospital Wise Amount Stats Report' +
