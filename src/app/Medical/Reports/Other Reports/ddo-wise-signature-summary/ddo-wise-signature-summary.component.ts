...
@Component({
  selector: 'app-ddo-wise-signature-summary',
  templateUrl: './ddo-wise-signature-summary.component.html',
  styleUrls: ['./ddo-wise-signature-summary.component.css'],
})
export class DdoWiseSignatureSummaryComponent {
  formTitle = ' DDO Wise Signature Summary Report';
  userId: any;
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  columns: string[][] = [['DDO_HEAD_OF_OFFICE', 'DDO Of The Official']];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.searchText != '') {
      likeQuery = " AND DDO_HEAD_OF_OFFICE like '%" + this.searchText + "%'";
    }
  ...
  convertInExcel() {
    ...
    if (this.inspector.length > 0) {
      for (var i = 0; i < this.inspector.length; i++) {
        obj1['Head Of Office'] = this.inspector[i]['DDO_HEAD_OF_OFFICE'];
        obj1['Signature Count'] = this.inspector[i]['SIGNATURE_COUNT'];

        arry1.push(Object.assign({}, obj1));
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'DDO Wise Signature Summary Report' +
