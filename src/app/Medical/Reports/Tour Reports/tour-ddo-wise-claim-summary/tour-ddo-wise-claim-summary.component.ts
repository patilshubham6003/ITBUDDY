...
@Component({
  selector: 'app-tour-ddo-wise-claim-summary',
  templateUrl: './tour-ddo-wise-claim-summary.component.html',
  styleUrls: ['./tour-ddo-wise-claim-summary.component.css'],
})
export class TourDdoWiseClaimSummaryComponent {
  formTitle = 'Tour DDO Wise Claim Summary Report';
  userId: any;
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  columns: string[][] = [['DDO_OF_THE_OFFICIAL', 'DDO Of The Official']];
  ...
  applyFilter() {
    ...
    if (this.SELECTFROMDATE || this.SELECTTODATE) {
      ...
      if (this.SELECTFROMDATE && !this.SELECTTODATE) {
        ...
        this.filterQuery +=
          " AND date(CREATED_DATE_TIME) BETWEEN '" +
          FromDate +
          "' AND '" +
          ToDate +
          "' ";
    ...
    if (this.CLAIM_AT) {
      this.filterQuery += ' AND IS_ADMIN_TOUR = ' + this.CLAIM_AT;
    }
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.searchText != '') {
      likeQuery = " AND DDO_OF_THE_OFFICIAL like '%" + this.searchText + "%'";
    }
  ...
  convertInExcel() {
    ...
    if (this.inspector.length > 0) {
      for (var i = 0; i < this.inspector.length; i++) {
        ...
        obj1['Total Count'] = this.inspector[i]['TOTAL'];
        obj1['Pending Count'] = this.inspector[i]['PENDING'];
        obj1['Approved Count'] = this.inspector[i]['APPROVED'];
        obj1['Rejected Count'] = this.inspector[i]['REJECTED'];
        obj1['On Hold Count'] = this.inspector[i]['ONHOLD'];
        arry1.push(Object.assign({}, obj1));
        if (i == this.inspector.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Tour DDO Wise Claim Summary Report' +
