...
@Component({
  selector: 'app-ltcmasterlist',
  templateUrl: './ltcmasterlist.component.html',
  styleUrls: ['./ltcmasterlist.component.css'],
  providers: [NzNotificationService],
})
export class LtcmasterlistComponent implements OnInit {
  drawerData2: any[] = [];
  ...
  search(reset: boolean = false) {
    ...
    if (!this.isAdmin) {
      ...
      this.extraFilterQuery =
        " AND IS_DELETED = 0 AND LTC_STATUS != 'P' AND IS_ADVANCE_CLAIM = 0 " +
        tempFilter;
    } else {
      // this.extraFilterQuery = "AND LTC_STATUS != 'P' ";
      this.extraFilterQuery = ' AND IS_DELETED = 0 AND IS_ADVANCE_CLAIM = 0';
    }
  ...
  edit(data: LTCMaster): void {
    ...
    //     this.drawerData.DDO_OF_THE_OFFICIAL_ID;
    // }
    this.drawerData.SALUTATION = this.drawerData.SALUTATION;
    if (
  ...
  advCalculationSheet(data) {
    ...
    if (this.advformdata7.length > 0) {
      ...
      for (let i = 0; i < this.advformdata7.length; i++) {
        ...
        advAmountClaimed += Number(
          this.advformdata7[i].JOURNEY_CLAIMED_AMOUNT *
            this.advformdata7[i].NO_OF_FAIRS
        );
      }
      ...
      .subscribe((journeyData) => {
        if (journeyData['code'] == 200) {
          ...
          if (journeyData['data'].length > 0) {
            ...
            eligibleAmount = Math.round((advAmountClaimed * 90) / 100);

            if (this.advGAR37Data.REQUIRED_ADVANCE_AMOUNT > eligibleAmount) {
              this.GAR37AdvanceAmount = eligibleAmount;
            } else if (
              this.advGAR37Data.REQUIRED_ADVANCE_AMOUNT < eligibleAmount
            ) {
              this.GAR37AdvanceAmount =
                this.advGAR37Data.REQUIRED_ADVANCE_AMOUNT;
            } else {
              this.GAR37AdvanceAmount =
                this.advGAR37Data.REQUIRED_ADVANCE_AMOUNT;
            }
