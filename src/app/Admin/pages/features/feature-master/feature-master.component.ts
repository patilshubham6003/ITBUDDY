...
export class FeatureMasterComponent implements OnInit {
  formTitle = 'Manage Features Information';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  drawerTitle: string;
  drawerData: Feature = new Feature();
  ...
  add(): void {
    this.drawerTitle = 'Create New Features';
    this.drawerData = new Feature();
    this.api.getAllFeature(1, 1, 'SEQUENCE_NO', 'desc', '').subscribe(
      (data) => {
        if (data['count'] == 0) {
  ...
  edit(data: Feature): void {
    this.drawerTitle = 'Update Features Information';
