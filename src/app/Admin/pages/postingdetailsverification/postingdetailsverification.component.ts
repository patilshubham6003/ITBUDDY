...
@Component({
  selector: 'app-postingdetailsverification',
  templateUrl: './postingdetailsverification.component.html',
  styleUrls: ['./postingdetailsverification.component.css'],
  providers: [DatePipe],
})
export class PostingdetailsverificationComponent {
  formTitle: any = 'Manage Posting Verification';
  ...
  search(reset: boolean = false) {
    ...
    if (sessionStorage.getItem('roleId') == '54') {
      filextra =
        ' AND RANK_ID IN (11,17,44,22,21,20,25,19,12,39,40,16,13,43,23,6,34,41,37,38)';
    }
    this.api
      .getallpostingreq(
        this.pageIndex,
      ...
      .subscribe((data) => {
        if (data['code'] == 200) {
          ...
          this.api.getallpostingreqcount(0, 0, '', '', '' + filextra).subscribe(
            (forms) => {
  ...
  approverejectdata(data: any, status: any) {
    ...
    var filter =
      ' AND EMPLOYEE_ID=' +
      this.verifydata.EMPLOYEE_ID +
      ' AND ID=' +
      this.verifydata.POSTING_DETAILS_ID;
    this.api.getAllPostingDetailsMaster(0, 0, '', '', filter).subscribe(
      (forms) => {
        if (forms['code'] == 200 && forms['data'].length > 0) {
          this.oldpostindata = forms['data'][0];
        } else {
          this.oldpostindata = [];
        }
  ...
  verify(data: any, status: any) {
    if (
      ...
      this.verifydata.REMARK = this.REMARK ? this.REMARK : null;
      ...
      // var updatedata={
      //   ID:this.verifydata.POSTING_DETAILS_ID,
      //   ...this.verifydata
      // }

      if (this.oldpostindata.length > 0) {
        this.oldpostindata.TO_DATE = this.verifydata.TO_DATE;
        this.oldpostindata.FROM_DATE = this.verifydata.FROM_DATE;
        this.oldpostindata.ORDER_TYPE = this.verifydata.ORDER_TYPE;
        this.oldpostindata.RANK_NAME = this.verifydata.RANK_NAME;
        this.oldpostindata.OFFICE_NAME = this.verifydata.OFFICE_NAME;
      }
      this.api
        .updatepostingrequest(this.verifydata)
        .subscribe((successCode) => {
          if (successCode['code'] == '200') {
            if (status == 'A') {
              ...
              if (this.oldpostindata.length > 0) {
                this.api
                  .updatepostingdetails(this.oldpostindata)
                  .subscribe((successCodeeee) => {
                    if (successCodeeee['code'] == '200') {
                      this.loadingRecords = false;
                      this.openmodell = false;
                    } else {
                      this.message.error(
