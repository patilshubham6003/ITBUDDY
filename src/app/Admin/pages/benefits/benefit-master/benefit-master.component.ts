...
export class BenefitMasterComponent implements OnInit {
  formTitle = 'Manage Benefits Information';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  drawerTitle: string;
  drawerData: Benefits = new Benefits();
  ...
  add(): void {
    this.drawerTitle = 'Create New Benefits';
    this.drawerData = new Benefits();
    this.api.getAllBenefit(1, 1, 'SEQUENCE_NO', 'desc', '').subscribe(
      (data) => {
        if (data['count'] == 0) {
  ...
  edit(data: Benefits): void {
    this.drawerTitle = 'Update Benefits Information';
