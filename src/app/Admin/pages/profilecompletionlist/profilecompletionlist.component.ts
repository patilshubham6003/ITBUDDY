...
@Component({
  selector: 'app-profilecompletionlist',
  templateUrl: './profilecompletionlist.component.html',
  styleUrls: ['./profilecompletionlist.component.css'],
  providers: [DatePipe],
})
export class ProfilecompletionlistComponent {
  formTitle: any='Profile Completion List'
  ...
  drawerClosess(): void {
    this.search(true);
    sessionStorage.removeItem('employeeeidddd')
  ...
  drawerDatas: any = [];
  drawerTitles: string = 'View Employee Details';
  ...
  openprofile(data) {
    this.loadingRecords=true
    var dddata=data
    sessionStorage.setItem('employeeeidddd',data.ID)
    this.drawerTitles = 'Profile Details';
    this.addnew = true
    this.loadbutton = true;
    this.api.getprofiledata(0, 0, '', '', ' AND ID=' + dddata.ID + '').subscribe(
      (data) => {
        if (data.code == '200' && data.data.length > 0) {
          var dataforper = data['data'][0];
          this.drawerDatapersonal = Object.assign({}, dataforper);
          this.api.getempstatus(0, 0, '', 'asc', ' AND ID=' + dddata.ID).subscribe((data) => {
            if (data['code'] == 200) {
