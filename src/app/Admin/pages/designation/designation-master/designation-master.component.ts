...
export class DesignationMasterComponent implements OnInit {
  show: boolean = false;
  formTitle = 'Manage Designation Information';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  columns: string[][] = [['NAME', 'Designation']];
  //drawer Variables
  ...
  add(): void {
    ...
    this.drawerData = new Designation();
    this.api.getAllDesignation(1, 1, 'SEQUENCE_NO', 'desc', '').subscribe(
      (data) => {
        if (data['count'] == 0) {
  ...
  edit(data: Designation): void {
    this.show = true;
    this.drawerTitle = 'Update Designation Information';
