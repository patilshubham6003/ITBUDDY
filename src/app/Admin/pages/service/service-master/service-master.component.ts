...
export class ServiceMasterComponent implements OnInit {
  formTitle = 'Manage Service Information';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  drawerTitle: string;
  drawerData: Service = new Service();
  constructor(
  ...
  edit(data: Service): void {
    ...
    this.drawerTitle = 'Update Service Information';
    this.drawerData = Object.assign({}, data);
    if (
      this.drawerData.ROLE_DATA != null &&
      this.drawerData.ROLE_DATA != undefined
    ) {
      this.drawerData.ROLE_DATA = this.drawerData.ROLE_DATA.split(',');
      for (var i = 0; i < this.drawerData.ROLE_DATA.length; i++) {
        this.drawerData.ROLE_DATA[i] = Number(this.drawerData.ROLE_DATA[i]);
      }
    } else {
      this.drawerData.ROLE_DATA = this.drawerData.ROLE_DATA;
