...
@Component({
  selector: 'app-jaosummaryreport',
  templateUrl: './jaosummaryreport.component.html',
  styleUrls: ['./jaosummaryreport.component.css'],
  providers: [DatePipe],
})
export class JaosummaryreportComponent {
  formTitle = 'Group Wise Summary';
  fileName = 'Group Wise Summary.xlsx';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (
      ...
      caderfilter = ' AND RANK_ID in (' + this.OFFICE_ID + ')';
    }
    ...
    if (exportInExcel == false) {
      ...
      this.api
        .getjaowisedata(0, 0, this.sortKey, sort, likeQuery + caderfilter)
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  sort(params: NzTableQueryParams): void {
    ...
    const sortField = (currentSort && currentSort.key) || 'GROUP_NAME';
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.dataList1.length; i++) {
      obj1['Group Name'] = this.dataList1[i]['GROUP_NAME'];
