...
@Component({
  selector: 'app-upcomingretirmentreport',
  templateUrl: './upcomingretirmentreport.component.html',
  styleUrls: ['./upcomingretirmentreport.component.css'],
  providers: [DatePipe],
})
export class UpcomingretirmentreportComponent {
  formTitle = 'Upcoming Retirements ';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (
      ...
      dataOfficecatb += ' AND CATEGORY_ID IN ' + '(' + this.category + ')';
    ...
    if (
      ...
      dataOfficeregist +=
        " AND date(RETIREMENT_DATE) > " +
        "'" +
        this.datepipe.transform(new Date(), 'yyyy-MM-dd') +
        "'";
      // dataOfficeregist= " AND RETIREMENT_DATE BETWEEN '" +
      //   this.datepipe.transform('2025-03-01', 'yyyy-MM-dd') +
      //   "' AND '" +
      //   this.datepipe.transform('2026-03-31', 'yyyy-MM-dd') +
      //   "'";
    ...
    for (var i = 0; i < this.excelData.length; i++) {
      ...
      if (i == this.excelData.length - 1) {
        this._exportService.exportExcel(
          arry1,
          'Upcoming Retirements Employee Details"' +
