...
@Component({
  selector: 'app-caderwisesummaryreport',
  templateUrl: './caderwisesummaryreport.component.html',
  styleUrls: ['./caderwisesummaryreport.component.css'],
  providers: [DatePipe],
})
export class CaderwisesummaryreportComponent {
  formTitle = 'Cadre Wise Strength Summary';
  fileName = 'Cadre Wise Strength Summary.xlsx';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  ...
  columns1: string[][] = [['NAME', 'Office Name']];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    // if(this.OFFICE_ID){
    //   this.OFFICE_ID=this.OFFICE_ID.join(',')
    // }
    var dataOffice = '';
    if (
      this.OFFICE_ID != null &&
      ...
      dataOffice += ' AND OFFICE_ID IN ' + '(' + this.OFFICE_ID + ')';
    }
    ...
    if (exportInExcel == false) {
      ...
      this.api.gercaderwisedata(0, 0, '', '', likeQuery + dataOffice).subscribe(
        (data) => {
          if (data['code'] == 200) {
  ...
  sort(params: NzTableQueryParams): void {
    ...
    const sortField = (currentSort && currentSort.key) || 'RANK_NAME';
