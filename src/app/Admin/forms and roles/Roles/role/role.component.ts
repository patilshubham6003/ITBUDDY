...
export class RoleComponent implements OnInit {
  ...
  loadRoles() {
    ...
    var Filter = '';
    if (this.roleId != '1') Filter = ' AND ID=' + this.roleId;
    else Filter = ' AND PARENT_ID=0';
    this.api.getAllRoles(0, 0, '', '', Filter).subscribe(
      (roles) => {
    ...
    if (this.data.NAME != undefined && this.data.NAME != '') {
      if (/^[a-zA-Z\s-, ]*$/.test(this.data.NAME) == true) {
        ...
        if (this.data.START_PAGE.startsWith('/')) {
          this.api.getAllForms(0, 0, 'ID', 'desc', '').subscribe(
            (data) => {
              ...
              if (filterData.length > 0) {
                ...
                if (this.data.ID) {
                  ...
                  this.api.createRole(this.data).subscribe((successCode) => {
                    if (successCode['code'] == '200') {
                      ...
                      sessionStorage.setItem('ROLE_ID', successCode['ROLE_ID']);
                      this.api
                        .getNewRole(0, 0, '', '', '')
                        .subscribe((data) => {
                          if (data['code'] == '200') {
                ...
                this.message.error(
                  'Your Entered StartPage is not present in Database',
                  ''
                );
              }
            },
