...
@Component({
  selector: 'app-ithr-main',
  templateUrl: './ithr-main.component.html',
  styleUrls: ['./ithr-main.component.css'],
  providers: [DatePipe],
})
export class IthrMainComponent implements OnInit {
  ...
  search(reset: boolean = false) {
    ...
    this.api
      .getempstatus(
        0, 0, '', 'asc', ' AND ID=' + sessionStorage.getItem('userId')
      )
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
            if (data['data'].length > 0 && data['data'][0].IS_PERSONAL_FILLED == 1 && data['data'][0].IS_ADDRESS_FILLED == 1 && data['data'][0].IS_SERVICE_FILLED == 1 && data['data'][0].IS_FAMILY_FILLED == 1 && data['data'][0].IS_OPTIONAL_FILLED == 1) {
