...
export class TrainingVideosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.servDEtailcondition == 'T') {
      ...
      if (this.filterQuery != '') {
        this.api.getdownloads(0, 0, 'CREATED_MODIFIED_DATE', 'desc', " AND STATUS=1 AND FILE_TYPE='TV'" + this.filterQuery).subscribe((data) => {
          if (data['code'] == 200) {
    ...
    this.api
      ...
      .subscribe((data) => {
        if (data['code'] == 200) {
          ...
          this.typeTV = data['data'];
          this.ncounts = data['count'];
          for (var i = 0; i < this.typeTV.length; i++) {
            this.drscti[i] = this.typeTV[i].VIDEO_DESCRIPTION;
          }
        }
      });
  }
  ...
  searchBlogs(event: any) {
    ...
    this.api.getdownloads(0, 0, 'CREATED_MODIFIED_DATE', 'desc', " AND STATUS=1 AND FILE_TYPE='TV'" + filter).subscribe((data) => {
      if (data['code'] == 200) {
        this.typeTV = data['data'];
        this.ncounts = data['count'];
        for (var i = 0; i < this.typeTV.length; i++) {
  ...
  clearfilt() {
    ...
    this.api.getdownloads(0, 0, 'CREATED_MODIFIED_DATE', 'desc', " AND STATUS=1 AND FILE_TYPE='TV'").subscribe((data) => {
      if (data['code'] == 200) {
        this.typeTV = data['data'];
        this.ncounts = data['count'];
        for (var i = 0; i < this.typeTV.length; i++) {
