...
@Component({
  selector: 'app-familydetail-profile',
  templateUrl: './familydetail-profile.component.html',
  styleUrls: ['./familydetail-profile.component.css']
})
export class FamilydetailProfileComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.data.RELATION != 'Daughter' && this.data.RELATION != 'Son') {
      ...
      this.disabledDate18years = (current: Date): boolean => {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        return (current >= new Date(today.getFullYear() - 18, today.getMonth(), today.getDate())
        );
  ...
  save(addNew: boolean, form1: NgForm): void {
    ...
    if (this.data.NAME == null || this.data.NAME == undefined || this.data.NAME == '') {
    ...
    } else if ((this.data.RELATION == 'Daughter' || this.data.RELATION == 'Son') && (this.data.DOB == null || this.data.DOB == undefined || this.data.DOB == '')) {
      ...
      if (this.data.NAME != undefined && this.data.NAME != '') {
        if (this.data.ID) {
          ...
          this.api.updateFamily(this.data).subscribe((successCode) => {
            if (successCode['code'] == '200') {
              ...
              this.api.updateprofile(this.personalinformation).subscribe((successCode) => {
                if (successCode['code'] == '200') {
                  this.isSpinning = false;
                } else {
                  this.isSpinning = false;
          ...
          this.data.EMPLOYEE_ID = sessionStorage.getItem('userId');
          this.api.createFamily(this.data).subscribe((successCode) => {
            if (successCode['code'] == '200') {
  ...
  getdata() {
    if (this.ID) {
      this.api.getFamily(0, 0, '', '', ' AND ID =' + this.ID).subscribe((data) => {
  ...
  add(): void {
    this.api.getFamily(0, 0, '', '', '').subscribe((data) => {
